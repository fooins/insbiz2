...
const { genClaimNo } = require('../../../src/components/claims/services/apply');
const {
  genPolicyNo,
...
const genDependencies = async () => {
  // 创建销售渠道
  const producerCode = `TEST-QUERY-CLAIM-${Date.now()}`;
  ...
  // 创建保险产品
  const productCode = `TEST-QUERY-CLAIM-${Date.now()}`;
  ...
  // 创建保险计划
  const planCode = `TEST-QUERY-CLAIM-${Date.now()}`;
  ...
  // 创建授权契约
  const contractCode = `TEST-QUERY-CLAIM-${Date.now()}`;
...
describe('查询理赔单接口', () => {
  test('当查询成功时，应得到理赔单数据', async () => {
    ...
    let authStr = '';
    {
      ...
      // 构造理赔单
      claim = await getClaimModel().create({
