...
const genDependencies = async () => {
  // 创建销售渠道
  const producerCode = `TEST-RENEW-${Date.now()}`;
  ...
  // 创建保险产品
  const productCode = `TEST-RENEW-${Date.now()}`;
  ...
  // 创建保险计划
  const planCode = `TEST-RENEW-${Date.now()}`;
  ...
  // 创建授权契约
  const contractCode = `TEST-RENEW-${Date.now()}`;
...
beforeAll(async () => {
  ...
  ctx.url = `http://${hostname}:${port}/v1.0/policies/renew`;
...
describe('续保接口', () => {
  test('当续保成功时，应得到新的保单号', async () => {
    ...
    let authStr = '';
    {
      ...
      // 开启续保
      ctx.bizConfig.renew.allowRenew = true;
    ...
    expect(rst).toMatchObject({
      status: 200,
      data: {
        orderNo: expect.not.stringContaining(policy.orderNo),
        policyNo: expect.stringMatching(
          new RegExp(`^${policy.policyNo}-[0-9]*$`),
        ),
        ...
        applicants: [
          {
            no: expect.not.stringContaining(applicant.no),
            name: applicant.name,
        ...
        insureds: [
          {
            no: expect.not.stringContaining(insured.no),
