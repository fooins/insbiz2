...
const genDependencies = async () => {
  // 创建销售渠道
  const producerCode = `TEST-ENDORSE-${Date.now()}`;
  ...
  // 创建保险产品
  const productCode = `TEST-ENDORSE-${Date.now()}`;
  ...
  // 创建保险计划
  const planCode = `TEST-ENDORSE-${Date.now()}`;
  ...
  // 创建授权契约
  const contractCode = `TEST-ENDORSE-${Date.now()}`;
...
const clearnTestDatas = async () => {
  ...
  // 删除保单快照
  await getPolicySnapshootModel().destroy({
    where: {
      policyId: {
        [Op.in]: policyIds,
      },
  ...
  // 查询需要删除的批单
  const endorsements = await getEndorsementModel().findAll({
    attributes: ['id'],
    where: {
      policyId: {
        [Op.in]: policyIds,
  ...
  const endorseIds = endorsements.map((e) => e.id);

  // 删除批单
  await getEndorsementModel().destroy({
    where: {
      id: {
        [Op.in]: endorseIds,
      },
  ...
  // 删除批单详情
  await getEndorsementDetailModel().destroy({
    where: {
      endorsementId: {
        [Op.in]: endorseIds,
      },
...
beforeAll(async () => {
  ...
  ctx.url = `http://${hostname}:${port}/v1.0/endorsements`;
...
describe('批改接口', () => {
  test('当批改成功时，应得到批单号', async () => {
    ...
    let authStr = '';
    {
      ...
      // 开启批改
      ctx.bizConfig.endorse.allowEndorse = true;
      ctx.bizConfig.endorse.policy.allowEndorse = true;
      ctx.bizConfig.endorse.policy.effectiveTime.allowEndorse = true;
      ctx.bizConfig.endorse.applicants.allowEndorse = true;
      ctx.bizConfig.endorse.applicants.name.allowEndorse = true;
      ctx.bizConfig.endorse.insureds.allowEndorse = true;
      ctx.bizConfig.endorse.insureds.name.allowEndorse = true;
      ...
      bodyStr = JSON.stringify({
        ...
        effectiveTime: moment(policy.effectiveTime)
          .add(1, 'minute')
          .toISOString(true),
        applicants: [
          {
            no: applicant.no,
            name: `${applicant.name}批`,
          },
        ],
        insureds: [
          {
    ...
    rst.data.details.forEach((detail) => {
